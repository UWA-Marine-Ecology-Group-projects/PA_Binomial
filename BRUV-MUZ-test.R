...
## Set file name --
filen <- "stereo-BRUVs-MUZ-seag-epower.csv"
...
dataComponents<-supplyData(
  ...
  scenario.data=list(
    ...
    filename="BRUV-MUZ-400it_binomal",
...
memfree #  5392800 kb - at the start of sesson : 64877516: after 100 its: 61748628

x <- system('grep MemTotal /proc/meminfo', intern = TRUE)
x # 65965264 kB # 200 its

# memory usage ----
library(pryr)
mem_used() # 1.19 GB # after cleaning: 165 MB, at the start of sess 48.5 MB, after 100 its: 272 MB - after 200 its : 424 MB: after 500 : 1.73 GB

# To obtain information on the total and/or on the available memory in linux, you can try
system('free -m') # 59419 200 its - 500 its: 11967
